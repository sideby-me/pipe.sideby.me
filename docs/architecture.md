...
pipe.sideby.me is a Cloudflare Worker that proxies video streams for the main Sideby.me app. It handles CORS, HLS manifest rewriting, SSRF protection, and range-request synthesis—all at the edge. It does not spoof request headers and is intended as a CORS/protocol helper when direct browser fetches are not possible.
...
- **Proxy Core** (`src/lib/proxy.ts`) — Fetches the upstream resource with retry logic, synthesizes byte-range responses, and delegates HLS rewrites.
...
- **Headers** (`src/lib/headers.ts`) — Builds forward headers with an honest User-Agent and optional pass-through Referer/Origin.
...
4. **Upstream fetch** — Fetch with pass-through headers; retry with minimal headers on 403.
5. **Content handling**:
